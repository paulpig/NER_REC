...
import os
import sys
from load_data_hypergraph import Data
import numpy as np
import math
...
class Model(object):
    ...
    def __init__(self, data_config):
        ...
        self.adj_mat_item2entity_symmentric = config['adj_mat_item2entity_symmentric']
        ...
        # overwrite item embedding.
        self.weights['item_embedding'] = tf.split(self._create_norm_embed_item2entity()[-1], [self.n_items, self.n_entity])[0]# last layer

        self.all_embeddings = self._create_norm_embed() #(user+item, dim)
        ...
        # # other branch
        # self.ua_embeddings_4_all, self.entity_embedding_4_all = tf.split(self.all_embeddings_entity[-1], [self.n_users, self.n_entity], 0) # sum layer
        # # conduct item embedding from attribute
        # self.item_embeddings_4_all = tf.sparse_tensor_dense_matmul(self._convert_sp_mat_to_sp_tensor(self.adj_mat_item2entity), self.entity_embedding_4_all) # (n_item, dim)
        # self.u_g_embeddings_4_all = tf.nn.embedding_lookup(self.ua_embeddings_4_all, self.users)
        # self.pos_i_g_embeddings_4_all = tf.nn.embedding_lookup(self.item_embeddings_4_all, self.pos_items)
        # self.neg_i_g_embeddings_4_all = tf.nn.embedding_lookup(self.item_embeddings_4_all, self.neg_items)

        # for inference, add other branch;
    ...
    def _create_norm_embed_item2entity(self):
        A_fold_hat = self._split_A_hat(self.adj_mat_item2entity_symmentric)

        ego_embeddings = tf.concat([self.weights['item_embedding'], self.weights['e_embedding']], axis=0)
        ...
        for k in range(0, self.n_layers):
            ...
            u_g_embeddings_4, i_g_embeddings_4 = tf.split(ego_embeddings, [self.n_items, self.n_entity], 0)
            # all_embeddings[k] = [u_g_embeddings_4, i_g_embeddings_4]
            all_embeddings.append(tf.concat([u_g_embeddings_4, i_g_embeddings_4], axis=0))
    ...
    def _create_norm_embed_entity(self):
        A_fold_hat = self._split_A_hat(self.entity_norm_adj) #(item, user + entity)
        tp_A_fold_hat = self._split_A_hat(self.entity_norm_adj.T)
        ...
        for k in range(0, self.n_layers):
            ...
            side_embeddings = self.FC(side_embeddings, self.emb_dim, name="fc", activation='leaky_relu', layer_idx=k) #(item, dim)
...
if __name__ == '__main__':
    ...
    norm, norm_entity, adj_mat_item2entity, adj_mat_item2entity_symmentric = data_generator.get_adj_mat() # 3, 4 mean that the matrix with different normalization weight.

    config['norm_adj'] = norm
    config['all_norm_adj'] = norm_entity
    config['adj_mat_item2entity'] = adj_mat_item2entity
    config['adj_mat_item2entity_symmentric'] = adj_mat_item2entity_symmentric
