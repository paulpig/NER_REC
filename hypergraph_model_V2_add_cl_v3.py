...
import os
import sys
from load_data_hypergraph_V3 import Data
import numpy as np
import math
...
class Model(object):
    ...
    def __init__(self, data_config):
        ...
        self.adj_mat_hypergraph_ia_u = config['adj_mat_hypergraph_ia_u']
        ...
        # v3: reduce_mean after l2 normalizing.
        # self.all_embeddings = [self._create_norm_embed_norm(merge_type='mean')]
        
        # pdb.set_trace()
        # self.all_embeddings_entity, _ = self._create_norm_embed_entity()
        self.all_embeddings_ui_a_all = [self._create_norm_embed_hypergraph_ui_a(hypergraph_adj=self.adj_mat_hypergraph_ui_a, fc_name="ui_a")] #(n_users + n_items + n_entity, dim)
        
        self.all_embeddings_ia_u_all = [self._create_norm_embed_hypergraph_ui_a(hypergraph_adj=self.adj_mat_hypergraph_ia_u, fc_name="ia_u")] #(n_users + n_items + n_entity, dim)
        ...
        # ui-a
        self.ua_embeddings_4_all, self.ia_embeddings_4_all, self.entity_embedding_4_all = tf.split(self.all_embeddings_ui_a_all[-1], [self.n_users, self.n_items, self.n_entity], 0) # sum layer
        ...
        # ia-u
        self.ua_embeddings_4_ia_u, self.ia_embeddings_4_ia_u, self.entity_embedding_4_ia_u = tf.split(self.all_embeddings_ia_u_all[-1], [self.n_users, self.n_items, self.n_entity], 0) # sum layer
        self.u_g_embeddings_4_ia_u = tf.nn.embedding_lookup(self.ua_embeddings_4_ia_u, self.users)
        self.pos_i_g_embeddings_4_ia_u = tf.nn.embedding_lookup(self.ia_embeddings_4_ia_u, self.pos_items)
        self.neg_i_g_embeddings_4_ia_u = tf.nn.embedding_lookup(self.ia_embeddings_4_ia_u, self.neg_items)
        ...
        self.batch_ratings = tf.matmul(self.u_g_embeddings_4, self.pos_i_g_embeddings_4, transpose_a=False, transpose_b=True) +\
                ...
                tf.matmul(self.u_g_embeddings_4_ia_u, self.pos_i_g_embeddings_4_ia_u, transpose_a=False, transpose_b=True)
        
        #three graphs.
        self.loss, self.mf_loss, self.emb_loss = self.create_bpr_loss_three_bpr_loss()
        ...
        self.u_cl_loss_v1 = self.instance_level_cl_loss(self.u_g_embeddings_4, self.u_g_embeddings_4_ia_u)
        self.i_pos_cl_loss_v1 = self.instance_level_cl_loss(self.pos_i_g_embeddings_4, self.pos_i_g_embeddings_4_ia_u)
        self.i_neg_cl_loss_v1 = self.instance_level_cl_loss(self.neg_i_g_embeddings_4, self.neg_i_g_embeddings_4_ia_u)

        self.u_cl_loss_v2 = self.instance_level_cl_loss(self.u_g_embeddings_4_ia_u, self.u_g_embeddings_4_all)
        self.i_pos_cl_loss_v2 = self.instance_level_cl_loss(self.pos_i_g_embeddings_4_ia_u, self.pos_i_g_embeddings_4_all)
        self.i_neg_cl_loss_v2 = self.instance_level_cl_loss(self.neg_i_g_embeddings_4_ia_u, self.neg_i_g_embeddings_4_all)

        # # cl_w = 5.e-4 # cl_loss: 12700;
        # cl_w = 1.e-4 # cl_loss: 12700;
        cl_w = 1.e-5 # cl_loss: 12700;
        self.loss = self.loss + cl_w * (self.u_cl_loss + self.i_pos_cl_loss + self.i_neg_cl_loss +\
                                        self.u_cl_loss_v1 + self.i_pos_cl_loss_v1 + self.i_neg_cl_loss_v1 +\
                                        self.u_cl_loss_v2 + self.i_pos_cl_loss_v2 + self.i_neg_cl_loss_v2)
    ...
    def _create_norm_embed_norm(self, merge_type="sum"):
        A_fold_hat = self._split_A_hat_right(self.norm_adj)
        ego_embeddings = tf.concat([self.weights['user_embedding'], self.weights['item_embedding']], axis=0)
    ...
    def _create_norm_embed_hypergraph_ui_a(self, hypergraph_adj, fc_name="ui_a"):
        A_fold_hat = self._split_A_hat_right(hypergraph_adj) #(item, user + entity)
        tp_A_fold_hat = self._split_A_hat_right(hypergraph_adj.T)
        ...
        for k in range(0, self.n_layers):
            ...
            for f in range(self.n_fold):
                temp_embed.append(tf.sparse_tensor_dense_matmul(A_fold_hat[f], ego_embeddings)) 
            side_embeddings = tf.concat(temp_embed, 0) #(item, dim)            # add FC
            side_embeddings = self.FC(side_embeddings, self.emb_dim, name=fc_name + "_fc_1", activation='leaky_relu', layer_idx=k) #(item, dim)
            # side_embeddings = self.FC(side_embeddings, self.emb_dim, name="fc_2", activation='leaky_relu', layer_idx=k) #(item, dim)
            # side_embeddings = self.FC(side_embeddings, self.emb_dim, name="fc_3", activation='leaky_relu', layer_idx=k) #(item, dim)
    ...
    def create_bpr_loss_three_bpr_loss(self):
        ...
        pos_scores_4_ia_u = tf.reduce_sum(tf.multiply(self.u_g_embeddings_4_ia_u, self.pos_i_g_embeddings_4_ia_u),
                                         axis=1)
        neg_scores_4_ia_u = tf.reduce_sum(tf.multiply(self.u_g_embeddings_4_ia_u, self.neg_i_g_embeddings_4_ia_u),
                                         axis=1)
        ...
        regularizer_ia_u = tf.nn.l2_loss(self.u_g_embeddings_4_ia_u) + tf.nn.l2_loss(self.pos_i_g_embeddings_4_ia_u) + \
                          tf.nn.l2_loss(self.neg_i_g_embeddings_4_ia_u)

        emb_loss = (decay_1 * regularizer_mf + decay_2 * regularizer_all + decay_3 * regularizer_ia_u) / self.batch_size
        # emb_loss = (decay_1 * regularizer_mf) / self.batch_size
        ...
        loss_4_ia_u = tf.reduce_mean(tf.nn.softplus(-(pos_scores_4_ia_u - neg_scores_4_ia_u)))

        loss = mf_loss_4 + emb_loss + loss_4_all + loss_4_ia_u
        # loss = mf_loss_4 + emb_loss
...
if __name__ == '__main__':
    ...
    print('start, get_adj_mat')
    norm, norm_entity, adj_mat_item2entity, _, adj_mat_hypergraph_ui_a, adj_mat_hypergraph_ia_u = data_generator.get_adj_mat() # 3, 4 mean that the matrix with different normalization weight.
    print("finish, get_adj_mat.")
    ...
    config['adj_mat_hypergraph_ia_u'] = adj_mat_hypergraph_ia_u
